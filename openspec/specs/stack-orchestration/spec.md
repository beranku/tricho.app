# stack-orchestration Specification

## Purpose

The single source of truth for how every TrichoApp runtime service (PWA, `tricho-auth`, CouchDB, Traefik, mock OIDC) is composed, started, and torn down. Defines a one-command experience that works identically on a developer laptop, in CI, and in production — with only env vars and secrets changing between environments. The Makefile is the operator's surface; Docker Compose `profiles:` are the mechanism.

Source files: `compose.yml`, `Makefile`, `.env`, `.env.example`, `infrastructure/pwa/Dockerfile`, `infrastructure/couchdb/Dockerfile`, `infrastructure/couchdb/entrypoint.sh`.
