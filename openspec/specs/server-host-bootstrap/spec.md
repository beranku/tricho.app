# server-host-bootstrap Specification

## Purpose

Defines how a fresh Ubuntu 24.04 ARM64 host (e.g., `o3.tricho.app`) is prepared to host the server-deploy stack: idempotent installation of Docker + Compose v2 + SOPS + age + restic, hardened Docker daemon configuration, persistent data layout under `/srv/tricho/`, and a hardened self-hosted GitHub Actions runner registered as a systemd unit. The bootstrap is one-shot from the operator's view, but every step is guarded so re-running is a no-op. Source files: `infrastructure/server/install-host.sh`, `infrastructure/server/install-runner.sh`, `infrastructure/server/daemon.json`, `infrastructure/server/runner.service`, `.github/workflows/server-bootstrap.yml`.
