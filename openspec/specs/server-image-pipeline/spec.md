# server-image-pipeline Specification

## Purpose

Defines how the server-side container images (`tricho-auth` and the `couchdb` wrapper image) are built, tagged, signed, and verified before being pulled by the deploy host. Native ARM64 builds, immutable SHA tags, cosign keyless signing via GitHub OIDC, and mandatory verify on the deploy side. The PWA is explicitly out of scope — it ships to Cloudflare Pages, not to GHCR. Source files: `.github/workflows/build-server-images.yml`, `infrastructure/couchdb/Dockerfile`, `infrastructure/couchdb/tricho-auth/Dockerfile`, `infrastructure/server/sync/verify.sh`.
