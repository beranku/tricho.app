# restore-from-local-zip Specification

## Purpose

Restoring a vault from a local `.tricho-backup.zip` file that the user has on disk (typically from a previous device's encrypted backup export). The surface is reachable both pre-unlock (as a Step 3 sub-flow of the welcome wizard, when the user has lost their RS QR but still has the ZIP) and post-unlock (from Settings → "Obnovit ze zálohy"). Both entry points mount the same `RestoreFromZipScreen` component and require the original Recovery Secret to key the new vault before any ZIP-derived docs are written.

Source files: `src/components/RestoreFromZipScreen.tsx`, `src/lib/restore.ts`, `src/components/AppShell.tsx`.
