...
Each registered passkey is a device. The user needs to see which devices are linked to their account, name them, recognise the current device, revoke other devices, and (when the per-plan device limit is hit) either revoke or upgrade. The same `DeviceLimitScreen` component renders both pre-unlock (when the OAuth callback returns `deviceApproved: false`) and post-unlock (from Settings → "Zařízení"), authenticated by either an `oauthJwt` or a long-lived `tokenStore`.

Source files: `src/components/DeviceLimitScreen.tsx`, `src/components/Settings.tsx`, `src/components/AppShell.tsx`, `src/sync/sync.ts` (sync progress).
