...
After Step 2 completes the wizard MUST call `fetchVaultStateOverHttp(username, jwt)` (with the existing 5 s timeout). A non-null result MUST set `step3.flow = 'existing'`; a null result or a timeout MUST set `step3.flow = 'new'`. The wizard MUST NOT expose a user-facing toggle to switch flows.
...
- `webauthn`: render the success copy and "Aktivovat biometrii" CTA. Clicking it MUST call the existing `registerPasskey()` from `src/auth/webauthn.ts` (PRF if available, fallback otherwise) and on success mark `step3.completed = true` and set `currentStep = 'final'`.
...
#### Scenario: Activate biometrics completes Step 3 and shows the final card
- **GIVEN** Step 3 is on `webauthn`
- **WHEN** the user activates biometrics and `registerPasskey()` resolves
