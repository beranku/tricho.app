# server-backup-restore Specification

## Purpose

Defines how the deploy host's persistent state — per-environment CouchDB data and the edge Traefik ACME store — is backed up off-site, retained, and periodically validated. Restic to a TLS-protected remote repository (B2 / rsync.net), explicit acknowledgement that on-disk CouchDB state holds plaintext metadata even though user payloads are ciphertext, and a monthly automated restore drill into an isolated throwaway compose project to prove backups are usable. Source files: `infrastructure/server/backup/restic-backup.sh`, `infrastructure/server/backup/restore-drill.sh`, `infrastructure/server/backup/cron.d/`, `docs/server-deploy.md`.
