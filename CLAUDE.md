...
- `docs/` — `architecture.md`, `developer-guide.md`, `build-and-deploy.md`, `testing.md`, `secrets.md`, `voice-czech.md` + index `README.md`.
...
Per-package commands (`cd app && npm run dev | build | typecheck | test ...`; `cd web && npm run dev | build | typecheck | lint`) live in `docs/developer-guide.md`. Build + deploy runbook (merge-dist, validate-build, Cloudflare Pages, promote workflow, rollback) lives in `docs/build-and-deploy.md`.
...
3. **Two service workers, two scopes.** `/sw.js` (from `shared/`) is a 15-line thin SW with no caching. `/app/sw.js` (from vite-pwa) is the full Workbox SW with `registerType: 'prompt'`, `skipWaiting: false`, `clientsClaim: false`. Updates are never silent — the user is prompted. See `docs/architecture.md`.
...
For the capability index grouped by domain, see `openspec/README.md`.
...
- `docs/README.md` — index of developer-facing reference docs.
...
- `web/src/content/help/` — end-user help portal (rendered at `tricho.app/help`).
...
- `openspec/README.md` — capability index grouped by domain.
