  ...
  return (
    ...
    >
      ...
      */}
      ...
      <section
        style={{ padding: 16, borderRadius: 12, background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(0,0,0,0.06)' }}
        data-testid="settings-about-section"
      >
        <h3 style={{ margin: '0 0 8px' }}>{m.settings_about_heading()}</h3>
        <dl style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 12px', fontSize: 13, margin: 0 }}>
          <dt style={{ color: 'var(--ink-3, #888)' }}>{m.settings_about_version()}</dt>
          <dd style={{ margin: 0 }}><code data-testid="about-version">{__APP_VERSION__}</code></dd>
          <dt style={{ color: 'var(--ink-3, #888)' }}>{m.settings_about_built()}</dt>
          <dd style={{ margin: 0 }}>
            <time dateTime={__APP_BUILD_TIME__} data-testid="about-build">
              {new Intl.DateTimeFormat('cs-CZ', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(__APP_BUILD_TIME__))}
            </time>
          </dd>
          <dt style={{ color: 'var(--ink-3, #888)' }}>{m.settings_about_commit()}</dt>
          <dd style={{ margin: 0 }}><code data-testid="about-commit">{__APP_COMMIT__}</code></dd>
        </dl>
        <p style={{ marginTop: 12, fontSize: 13 }}>
          <a
            href={`https://github.com/honzaberanek/tricho-app/releases/tag/app-v${__APP_VERSION__}`}
            target="_blank"
            rel="noopener noreferrer"
            data-testid="about-release-notes-link"
          >
            {m.settings_about_release_notes_link()}
          </a>
        </p>
      </section>
